...
describe('Transporter Precompile - E2E', () => {
  ...
  const fundingAmount = 20n * 10n ** 18n
  ...
  describe('EVM to Consensus Transfer', () => {
    ...
    test('transfer with confirmations: 0 returns immediately', async () => {
      const smallAmount = 1n * 10n ** 18n // 1 AI3
      const result = await transferToConsensus(ethersWallet, consensusWallet.address, smallAmount, {
        confirmations: 0,
      })
